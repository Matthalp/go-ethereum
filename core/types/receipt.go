...
// AreMissingMetadata returns whether or not the receipts are missing their
// metadata fields.
func (r Receipts) AreMissingMetadata() bool {
	// The transaction hash was removed at the same time as GasCost, which
	// is hard to detect given that it's a uint64 (on mainnet it is impossible,
	// but could be the case on a private chain that changed gas costs).
	return len(r) > 0 && r[0].TxHash == common.Hash{}
}
